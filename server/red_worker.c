...
#include <sys/time.h>
typedef struct _time_cost_info 
{
...
#if 1
static TIME_COST_INFO time_cost_info;

#define TIME_STATICS_START()                                                                                          \
({                                                                                     \
        gettimeofday(&time_cost_info.start_time, NULL);                                                             \
})
#define TIME_STATICS_END(s)                                                                                            \
({                                                                                                               \
        gettimeofday(&time_cost_info.end_time, NULL);                                                               \
        printf("%s COST: %6d us\n",s,(time_cost_info.end_time.tv_sec-time_cost_info.start_time.tv_sec)*1000000+(time_cost_info.end_time.tv_usec-time_cost_info.start_time.tv_usec)); \
})
#endif

#define SPICE_MSG_DISPLAY_CCLIENT_DRAW 211
...
typedef struct CclientDrawItem {
    SpiceMsgCclientDraw cclient_draw;
    PipeItem pipe_item;
} CclientDrawItem;
...
typedef struct RedWorker {
    ...
    //zxs add
    int ClientType;     //cclient 0 other 1
    int UpdateCanvas;   //cavse is update 
...
#define red_update_area_till(worker, area, surface_id, last)
#else
...
#endif
//zxs add
static CclientDrawItem *get_cclient_draw_item(
    RedChannel* channel,uint32_t size,uint8_t *data, uint32_t format,int width,int height,uint32_t frames)
{
    CclientDrawItem *item;
    char *pH264Data;
    item = spice_malloc(sizeof(CclientDrawItem));

    item->cclient_draw.format = format;
    item->cclient_draw.num_frames = frames;
                                        ...
                                        uint32_t group_id)
{
...
#ifdef PIPE_DEBUG
    printf("TEST: DRAWABLE: %p red %p type %s effect %u bbox %u %u %u %u\n",
    ...
    */
    ...
    if(worker->ClientType == 0)
    {
    	worker->UpdateCanvas++;
    }
    else if(worker->ClientType == 1)
    {
    	if (red_current_add_qxl(worker, &worker->surfaces[surface_id].current, drawable,red_drawable)) 
    	{
        if (drawable->tree_item.effect != QXL_EFFECT_OPAQUE) 
        {
            worker->transparent_count++;
    ...
    *ring_is_empty = FALSE;
    while (!display_is_connected(worker) ||
        ...
        case QXL_CMD_DRAW: {
            ...
            if (!red_get_drawable(&worker->mem_slots, ext_cmd.group_id,red_drawable, ext_cmd.cmd.data, ext_cmd.flags)) 
            {
...
static void red_marshall_cclient_draw(RedChannelClient *rcc,
    SpiceMarshaller *base_marshaller, SpiceMsgCclientDraw *cclient_draw)
{
    red_channel_client_init_send_data(rcc, SPICE_MSG_DISPLAY_CCLIENT_DRAW, NULL);
    spice_marshall_msg_display_cclient_draw(base_marshaller,cclient_draw);
}    
...
static void display_channel_send_item(RedChannelClient *rcc, PipeItem *pipe_item)
{
    ...
    case PIPE_ITEM_TYPE_CCLIENT_DRAW:
    	{
    		CclientDrawItem *cclient_draw_item = SPICE_CONTAINEROF(pipe_item,CclientDrawItem, pipe_item);
    		//printf("i get here send %p \n",cclient_draw_item);
    		red_marshall_cclient_draw(rcc,m,&cclient_draw_item->cclient_draw);
    		break;
    	}
    default:
                                      ...
                                      void *line_0, int data_is_valid, int send_client)
{
    ...
    if (worker->renderer != RED_RENDERER_INVALID) {
        ...
        //zxs add
        worker->image = surface->context.canvas->ops->get_image(surface->context.canvas,TRUE);
        if (send_client) {
...
static int display_channel_init(DisplayChannelClient *dcc, SpiceMsgcDisplayInit *init_info)
{
    ...
    else
    {
    	...
    	worker->rgb_in.Info.Width          = MSDK_ALIGN16(worker->client_w);
      worker->rgb_in.Info.CropW          = worker->client_w;
      worker->rgb_in.Info.Height         = MSDK_ALIGN16(worker->client_h);
      worker->rgb_in.Info.CropH          = worker->client_h;
       
      worker->nv12_out.Info.Width          = MSDK_ALIGN16(worker->client_w);;
      worker->nv12_out.Info.CropW          = worker->client_w;
      worker->nv12_out.Info.Height         = MSDK_ALIGN16(worker->client_h);
      worker->nv12_out.Info.CropH          = worker->client_h;
    	
      memset(&par, 0, sizeof(par));
	    par.IOPattern = MFX_IOPATTERN_IN_SYSTEM_MEMORY|MFX_IOPATTERN_OUT_SYSTEM_MEMORY;
      par.AsyncDepth = 1;
      par.vpp.In = worker->rgb_in.Info;
      par.vpp.Out = worker->nv12_out.Info;
      
      err = MFXVideoVPP_Reset(worker->session,&par);
    	if (err != MFX_ERR_NONE) 
    	{
				printf("MFXVideoVPP_Reset error %d\n",err);
				exit(0);
    	}
    	av_dict_set(&param, "async_depth", "1", 0);  //µÍÑÓ³Ù¡£
                                       ...
                                       QXLDevSurfaceCreate surface)
{
    ...
    printf("file = '%s', line = %d fun = %s %dX%d \n", __FILE__, __LINE__ , __FUNCTION__ , surface.width, surface.height);
...
void *cclient_x264_thread(RedWorker *worker)
{
  ...
  if(w != oldw || h != oldh)
  {
  	 if(worker->img_convert_ctx != NULL)
  	 {
  	 	sws_freeContext(worker->img_convert_ctx);
  	 	worker->img_convert_ctx = NULL;
  	 }
  	 printf("cavse size %dx%d old %dx%d\n",w,h,oldw,oldh);

  	 oldw = w;
  	 oldh = h;
  	
  	 worker->img_convert_ctx = sws_getContext(w, h, AV_PIX_FMT_BGRA,  
                              worker->client_w, worker->client_h, AV_PIX_FMT_YUV420P,  
                              SWS_FAST_BILINEAR,   
                              NULL, NULL, NULL);
	...
	if(worker->UpdateCanvas)
	{
        ...
        //TIME_STATICS_END("RGB=>YUV");
    		...
    		{
    			//µÚÒ»Ö¡    			
    			int	picture_size = avpicture_get_size(worker->pCodecCtx->pix_fmt, worker->pCodecCtx->width, worker->pCodecCtx->height);
    			printf("picture_size %d w %d h %d \n",picture_size,worker->pCodecCtx->width, worker->pCodecCtx->height);
    			worker->picture_buf = (uint8_t *)av_malloc(picture_size);
    			avpicture_fill((AVPicture *)worker->pFrame, worker->picture_buf, worker->pCodecCtx->pix_fmt, worker->pCodecCtx->width, worker->pCodecCtx->height);
    			avformat_write_header(worker->pFormatCtx,NULL);
    			memset(&worker->pkt, 0, sizeof(AVPacket));
    			av_new_packet(&worker->pkt,picture_size);
    			worker->pFrame->width = worker->pCodecCtx->width;
    			worker->pFrame->height = worker->pCodecCtx->height;
    			worker->pFrame->format = worker->pCodecCtx->pix_fmt;
    			worker->pFrame->pict_type=AV_PICTURE_TYPE_I;
    			worker->pFrame->key_frame=1;
    		}
    		else if(iCountH264 == 2)
    		{
    		  worker->pFrame->pict_type=0;
    			worker->pFrame->key_frame=0;
    		}
   		
    		worker->pFrame->data[0] = dst_data[0];  // Y
    		worker->pFrame->data[1] = dst_data[1];  // U 
    		worker->pFrame->data[2] = dst_data[2];  // V
     		//PTS
    		worker->pFrame->pts=iCountH264;
    		got_picture=0;
    		//Encode
    		//TIME_STATICS_START();
    		ret = avcodec_encode_video2(worker->pCodecCtx, &worker->pkt,worker->pFrame, &got_picture);
    		//TIME_STATICS_END("YUV=>H264");
    		if(ret < 0)
    		{
    ...
    WORKER_FOREACH_DCC_SAFE(worker, dcc_ring_item, next, dcc) 
  	{	
    		if (got_picture==1)
    		{
    			//printf("Succeed to encode frame worker->video_st->index %d \n",worker->video_st->index);
    			worker->pkt.stream_index = worker->video_st->index;
...
void *cclient_qsv_thread(RedWorker *worker)
{
  ...
  int dst_linesize[4];
	...
	static int iNeedLastOne=0;
	int got_picture = 0;
	char *pBuff,*pTmp,*pTmp1;
	static TIME_COST_INFO time_cost;
	int cost_usec;
  mfxStatus err;
  mfxSyncPoint sync = NULL;
  int i;
	printf("\ncclient_qsv_thread tid=%lu, pid=%lu\n", getpid(), getpid());
	ret = av_image_alloc(dst_data, dst_linesize, worker->client_w, worker->client_h , AV_PIX_FMT_NV12, 1);  
  if (ret< 0) 
  {  
    printf( "Could not allocate destination image\n");  
    return false;  
  ...
  if(w != oldw || h != oldh)
  {
  	 ...
  	 worker->img_convert_ctx = sws_getContext(w, h, AV_PIX_FMT_BGRA,  
                              worker->client_w, worker->client_h, AV_PIX_FMT_NV12,  
                              SWS_FAST_BILINEAR,   
                              NULL, NULL, NULL);
    ...
    //		TIME_STATICS_START();
    //    gettimeofday(&time_cost.start_time, NULL);
    		pBuff = pixman_image_get_data(image);
        if(pBuff == NULL)
        {
        	return TRUE;
        }
    		...
    		pthread_mutex_lock (&worker->mutex); //zxs add
    		if( w != worker->client_w || h != worker->client_h )
    		{
    			sws_scale(worker->img_convert_ctx,src_data,src_linesize,0,h,dst_data,dst_linesize);
    		}
    		else
    		{  
    		    pTmp = pBuff;
    			  worker->rgb_in.Data.PitchLow = worker->client_w*4;
    				worker->rgb_in.Data.R        = pTmp;
    				worker->rgb_in.Data.G        = pTmp+worker->client_w*worker->client_h;
    				worker->rgb_in.Data.B        = pTmp+worker->client_w*worker->client_h*2;
    				worker->rgb_in.Data.A        = pTmp+worker->client_w*worker->client_h*3;
					
    				worker->nv12_out.Data.PitchLow = worker->client_w;
    				worker->nv12_out.Data.Y        = dst_data[0];
    				worker->nv12_out.Data.UV       = dst_data[1];
    				worker->nv12_out.Data.A       = NULL;

    			 do{
        			err =  MFXVideoVPP_RunFrameVPPAsync(worker->session, &worker->rgb_in, &worker->nv12_out , NULL, &sync);
    	...
    	//		printf("MFXVideoVPP_RunFrameVPPAsync error %d sync %x \n",err,sync);
    			do {
        		err = MFXVideoCORE_SyncOperation(worker->session, sync, 1000);
     //   		printf("MFXVideoCORE_SyncOperation error %d \n",err);
      ...
      //  TIME_STATICS_END("RGB=>YUV");
    		...
    		//TIME_STATICS_START();
    		ret = avcodec_encode_video2(worker->pCodecCtx, &worker->pkt,worker->pFrame, &got_picture);
    		//printf("encode end ret = %d got_picture %d \n",ret,got_picture);
    		//TIME_STATICS_END("YUV=>H264");
    		if(ret < 0)
    		{
    			printf("Failed to encode! \n");
    ...
    usleep(1000*30); //´óÔ¼Ã¿Ãë30Ö¡
	}
	else
	{
		if(iNeedLastOne == 1)
		{
		printf("draw last one frame \n");
		got_picture=0;
		avcodec_encode_video2(worker->pCodecCtx, &worker->pkt,worker->pFrame, &got_picture);
		WORKER_FOREACH_DCC_SAFE(worker, dcc_ring_item, next, dcc) 
  	{	
    		if (got_picture==1)
    		{
...
int decoder_deinit(RedWorker *worker)
{
	avcodec_close(worker->pCodecCtx);  
}
static int decoder_init_sw(RedWorker *worker)
{
	  //zxs add
    const char* out_file = "/tmp/ds.h264";


	  //Method1.
	  worker->pFormatCtx = avformat_alloc_context();
	  worker->fmt = av_guess_format("h264",out_file,NULL);
 	  worker->pFormatCtx->oformat = worker->fmt;
	  
#ifdef DUMP_H264
	  if (avio_open(&worker->pFormatCtx->pb,out_file, AVIO_FLAG_READ_WRITE) < 0)
	  {
	...
	worker->video_st = avformat_new_stream(worker->pFormatCtx, 0);
	worker->video_st->time_base.num = 1; 
	worker->video_st->time_base.den = 25;  

	if (worker->video_st==NULL)
	{
		printf("video_st == NULL \n");
		return -1;
	}
	//Param that must set
	worker->pCodecCtx = worker->video_st->codec;
	worker->pCodecCtx->codec_id = worker->fmt->video_codec;
	worker->pCodecCtx->codec_type = AVMEDIA_TYPE_VIDEO;
	worker->pCodecCtx->pix_fmt = AV_PIX_FMT_YUV420P;
	worker->pCodecCtx->width = worker->client_w;  
	worker->pCodecCtx->height = worker->client_h;
	worker->pCodecCtx->time_base.num = 1;  
	worker->pCodecCtx->time_base.den = 25;  
	worker->pCodecCtx->bit_rate = 2000000;  
	worker->pCodecCtx->gop_size= 250;
	//H264
	worker->pCodecCtx->me_range = 16;
	worker->pCodecCtx->max_qdiff = 4;
	worker->pCodecCtx->qcompress = 0.6;
	
	worker->pCodecCtx->qmin = 10;
	worker->pCodecCtx->qmax = 51;

	//Optional Param
	worker->pCodecCtx->max_b_frames = 0 ; //ÊµÊ±±àÂë Éè³É0 Ã»ÓÐ±àÂëÑÓ³Ù

	worker->pCodec = avcodec_find_encoder_by_name("libx264");
	if (!worker->pCodec)
	{
...
static int decoder_init_hw(RedWorker *worker)
{
    ...
    VAStatus va_res = VA_STATUS_SUCCESS;
	  mfxVersion ver = { { 3, 1 } };
	  mfxVideoParam par;
	  mfxFrameAllocRequest req_vpp[2];
    ...
    setenv("LIBVA_DRIVERS_PATH","/opt/intel/mediasdk/lib64",1);
    
    va_dpy = vaGetDisplayDRM(fd);
    va_res = vaInitialize(va_dpy, &major_version, &minor_version);
    err = MFXInit(impl, &ver, &worker->session);
    if (err != MFX_ERR_NONE) 
    {
				printf("MFXInit error %d\n",err);
				return -1;
    }

	  memset(&worker->rgb_in,0,sizeof(mfxFrameSurface1));
    worker->rgb_in.Info.FourCC         = MFX_FOURCC_RGB4;
    worker->rgb_in.Info.ChromaFormat = MFX_CHROMAFORMAT_YUV420;

    worker->rgb_in.Info.Width          = MSDK_ALIGN16(worker->client_w);
    worker->rgb_in.Info.CropW          = worker->client_w;
    worker->rgb_in.Info.Height         = MSDK_ALIGN16(worker->client_h);
    worker->rgb_in.Info.CropH          = worker->client_h;
    worker->rgb_in.Info.FrameRateExtN  = 30;
    worker->rgb_in.Info.FrameRateExtD  = 1;
    worker->rgb_in.Info.PicStruct = MFX_PICSTRUCT_PROGRESSIVE;
    
    memset(&worker->nv12_out,0,sizeof(mfxFrameSurface1));
    worker->nv12_out.Info.FourCC         = MFX_FOURCC_NV12;
    worker->nv12_out.Info.ChromaFormat = MFX_CHROMAFORMAT_YUV420;
    worker->nv12_out.Info.Width          = MSDK_ALIGN16(worker->client_w);;
    worker->nv12_out.Info.CropW          = worker->client_w;
    worker->nv12_out.Info.Height         = MSDK_ALIGN16(worker->client_h);
    worker->nv12_out.Info.CropH          = worker->client_h;
    worker->nv12_out.Info.FrameRateExtN  = 30;
    worker->nv12_out.Info.FrameRateExtD  = 1;
    worker->nv12_out.Info.PicStruct = MFX_PICSTRUCT_PROGRESSIVE;
    ...
    par.vpp.In = worker->rgb_in.Info;
    par.vpp.Out = worker->nv12_out.Info;
	  err = MFXVideoCORE_SetHandle(worker->session,(mfxHandleType)MFX_HANDLE_VA_DISPLAY,(mfxHDL)va_dpy);
    if (err < 0) 
    {
			printf("MFXVideoCORE_SetHandle error %d\n",err);
			return -1;
    }
    err = MFXVideoVPP_QueryIOSurf(worker->session,&par,req_vpp);
    if (err != MFX_ERR_NONE) 
    {
			printf("MFXVideoVPP_QueryIOSurf error %d\n",err);
			return -1;
    }    
    err = MFXVideoVPP_Init(worker->session, &par);
    if (err != MFX_ERR_NONE) 
    {
			printf("MFXVideoVPP_Init error %d\n",err);
			return -1;
    }
	...
	worker->video_st = avformat_new_stream(worker->pFormatCtx, 0);
	worker->video_st->time_base.num = 1; 
	worker->video_st->time_base.den = 30;  
	...
	worker->pCodecCtx->codec_id = worker->fmt->video_codec;
	worker->pCodecCtx->codec_type = AVMEDIA_TYPE_VIDEO;
	worker->pCodecCtx->pix_fmt = AV_PIX_FMT_NV12;
	worker->pCodecCtx->width = worker->client_w;  
	worker->pCodecCtx->height = worker->client_h;
	worker->pCodecCtx->time_base.num = 1;  
	worker->pCodecCtx->time_base.den = 30;  
	worker->pCodecCtx->bit_rate = 4000000;  
	worker->pCodecCtx->rc_max_rate = 4000000;
	worker->pCodecCtx->gop_size= 250;
	//Optional Param
	worker->pCodecCtx->max_b_frames = 0 ; //ÊµÊ±±àÂë Éè³É0 Ã»ÓÐ±àÂëÑÓ³Ù
 
	worker->pCodec = avcodec_find_encoder_by_name("h264_qsv");
	if (!worker->pCodec)
	{
		printf("Can not find encoder! \n");
		return -1; 
...
static void red_init(RedWorker *worker, WorkerInitData *init_data)
{
    ...
    //zxs add
    av_register_all();
    worker->client_w = 1366;
    worker->client_h = 768;
    i = decoder_init_hw(worker);
    if( i < 0 )
    {
    	//Èí¼þ±àÂë
    	printf("hw decoder init error try sw decoder\n");
    	i = decoder_init_sw(worker);
    	if(i<0)
    	{
    		printf("sw decoder init error \n");
    		exit(0);
    	}
    }
}
